/**
 * Model-text helper for this service: ask a model for one block of text and
 * keep the reason when there is none.
 *
 ...
 * indistinguishable from a model that legitimately returned nothing. This
 * service did exactly that, and reported every one of those conditions as
 * "the model produced no output".
 *
 * This is the same helper `dsh-memo` uses, kept structurally identical
 * (`packages/memo/dsh-memo/src/llm-text.ts`) so the two services report a
 * failure the same way. It is duplicated rather than shared because
 * `dsh-github-issue` is the lower layer — `dsh-memo` depends on it, not the
