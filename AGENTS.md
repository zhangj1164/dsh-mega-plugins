...
scripts/
  ...
  verify-package-exports.ts       # Declared exports/types targets must exist
  verify-bundle-entries.ts        # Bundle patch entry ids must not collide
.github/
...
pnpm run gates                        # full gate suite: build + test + hygiene + doc-sync + policy (9 gates)
...
pnpm run verify-bundle-entries        # no two workspace bundles insert the same composition entry id
...
- **Bundle layers**: `dsh-telemetry`, `dsh-github-issue` and `dsh-memo` each declare `dsh.bundle` with their own `cordis.patch.yml`, so any of them can be installed on its own. The memo bundle patch also inserts `dsh-github-issue` for deployments that want the suite; that insertion and the standalone one are interchangeable, not additive — a deployment that installs both must not enable both rows. DSH applies bundle layers in `dsh.profile.bundles` order with no transitive resolution, and a later layer cannot remove an earlier layer's insert, so two bundles inserting one entry id is a boot failure rather than a merge. `pnpm run verify-bundle-entries` enforces the part this repository can see: every entry id inserted by more than one workspace bundle must have **both** patches recording the exclusion with the marker `not both`. An undocumented overlap fails the gate. What no gate here can see is a deployment enabling two mutually exclusive patch layers through its own profile — that remains a constraint the deployment must honor.
